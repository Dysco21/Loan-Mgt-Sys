...
Class Action {
	...
	function save_borrower(){
		...
		if($_FILES['images']['tmp_name'] != ''){
						$images = strtotime(date('y-m-d H:i')).'_'.$_FILES['images']['name'];
						$move = move_uploaded_file($_FILES['images']['tmp_name'],'../assets/uploads/'. $images);
					$data .= ", images = '$images' ";
					$chk = $this->db->query("SELECT * FROM borrowers where images = '$images' ")->num_rows;
					if($chk->num_rows > 0){
					$save = $this->db->query("UPDATE borrowers set ".$data);
					}else{
					$save = $this->db->query("INSERT INTO borrowers where images = '$images' ".$data);
					}
		}
		if(empty($id)){
